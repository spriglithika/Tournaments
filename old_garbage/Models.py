...
class NeuralIsingTournamentFull(nn.Module):
    def __init__(self, num_classes, backbone = 'resnet18', device = 'cpu', learn_J=True, freeze_backbone=False, unfreeze_last_n=2):
        super().__init__()
        self.device = device
    ...
    def build_signed_adjacency(self, gamma=1.0, randomize=False):
        if randomize:
            torch.manual_seed(42)
            J = torch.randn((self.num_edges, self.num_edges))
            J = J / J.norm(p=2)
            return J
        ...
        for a, (i1, j1) in enumerate(edge_list):
            for b, (i2, j2) in enumerate(edge_list):
                if a == b:
                    J[a, b] = 1 # gamma  # self-interaction (optional)
                    continue
    ...
    def forward(self, x, max_iter=5, alpha = 0.2, train=False, tol=1e-4):
        # Extract features
        ...
        for _ in range(max_iter):
            ...
            m_up = ( 1- alpha) * m_prev + alpha * F.tanh(torch.matmul(m_prev, self.J))  # update rule
            # m = ( 1- alpha) * m + alpha * torch.tanh(torch.matmul(m, self.J))  # update rule
            # Optional: check for convergence
            # if torch.max(torch.abs(m_up - m_prev)) < tol:
                # break
