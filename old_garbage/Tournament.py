...
class Tournament(torch.nn.Module):
    ...
    def selectionIndicies(self):
        # Construct corner indicators and edge indices. The logic below can
        # produce empty tensors for very small numbers of classes (e.g., n=2),
        # which leads to reshape errors. Guard the complex construction and
        # fall back to a safe, minimal representation when needed.
        ...
        try:
            ...
            # Build cevians; index with `l` (locations of upper-triangular entries)
            cevians = torch.eye(self.num_classes**2)[edge_index[:, 0] * self.num_classes + edge_index[:, 1]]
            cevians = cevians.reshape((self.num_classes, self.euc_dim, self.num_classes**2))[:, :, l].reshape(self.num_classes, self.euc_dim, self.num_edges)
            ...
            # Fallback for degenerate/small cases (safe defaults)
            cevians = torch.zeros((self.num_classes, max(1, self.euc_dim), max(1, self.num_edges)), dtype=torch.float32)
            # idx is used later for gathering; provide a zero-filled int64 tensor
            idx = torch.zeros((self.n * max(1, self.d) * max(1, self.d),), dtype=torch.int64)
