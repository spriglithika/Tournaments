...
from TournamentThresholds import SingleConfidence, CenterThresholding
...
class NeuralIsingEnergyDeclarative(nn.Module):
    def __init__(self, num_classes, backbone = 'resnet18', device = 'cpu', freeze_backbone=False, unfreeze_last_n=2):
        super().__init__()
        self.device = device
    ...
    def forward(self, x, labels = None, train=True, max_iter=5, alpha = 0.1, tol=1e-4, T_mf = 1.0, T_sof = 1.0, ):
        h = self.middle(x)
