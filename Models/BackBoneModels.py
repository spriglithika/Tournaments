...
class BaseModel(torch.nn.Module):
    def __init__(self, num_classes, backbone= 'resnet18', device = 'cpu', freeze_backbone=False, unfreeze_last_n=1, softening=False):
    ...
    def forward(self, x, y = None, train = False):
        x = self.model(x)
        x = x / (self.log_Tsoft.exp())  # Apply softening to logits if enabled
