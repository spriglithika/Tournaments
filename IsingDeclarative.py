...
class IsingJBlock(nn.Module):
    ...
    def forward(self, h, *,
                ...
                damp: float = 1e-3):
        Q = self._compute_Q()
        return IsingMFStructuredWithQ.apply(
            h, self.A, self.AtA_inv, self.M_raw, self.S_raw, self.V_raw, Q,
            float(T), float(self.s_max), int(max_iter), float(alpha), float(tol),
            int(cg_max), float(cg_tol), float(damp)
        )
