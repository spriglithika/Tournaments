...
from old_garbage.Models import BaseModel, MidModel, TournamentModelDropout, copy_matching_parameters
from old_garbage.Training_testing_old import joint_train_all_mixup, joint_train_all, joint_eval_all, ConvergenceMonitor
...
def main(num_epochs, path_mod):
    ...
    tournament_model = TournamentModel(class_count, mode='solver', device = device, backbone='resnet18').to(device)
    ...
    ckpt_base = f'ckpts/cifar100/resnet18/solver{_path_mod}'
    monitor = ConvergenceMonitor(patience=3, mode='max', save_dir=ckpt_base)

    print("Starting joint training...")
    for epoch in range(num_epochs):
        print(f"Epoch {epoch}")
        # joint_train_all_mixup(device, train_loader, models, class_count, temps = [1,1, 1], lbda = 0.0)
        joint_train_all(device, train_loader, models, class_count, temps = [1,1, 1], lbda = 0.0)
