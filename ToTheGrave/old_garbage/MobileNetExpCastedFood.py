...
from old_garbage.Models import BaseModel, MidModel, TournamentModel, copy_matching_parameters
from old_garbage.Training_testing_old import joint_train_all, joint_eval_all, ConvergenceMonitor
...
def main(num_epochs, path_mod):
    train_dataset = datasets.Food101('../data', split='train', download=True,
    ...
    val_size = int(len(train_dataset) * .1)
    train_size = len(train_dataset) - val_size
    train_dataset, val_dataset = torch.utils.data.random_split(train_dataset, [train_size, val_size])
    test_dataset = datasets.Food101('../data', split='test',
                                transform=transforms.Compose([
                                    transforms.ToTensor(),
                                    transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
                                ]))
    ...
    class_count = len(test_dataset.classes) + 1
    ...
    # optimizer_base = torch.optim.AdamW(base_model.parameters(), lr=0.005)
    # optimizer_mid = torch.optim.AdamW(mid_model.parameters(), lr=0.005)
    # optimizer_tournament = torch.optim.AdamW(tournament_model.parameters(), lr=0.005)
    ...
    ckpt_base = f'ckpts/food101/mobilenet{_path_mod}'
