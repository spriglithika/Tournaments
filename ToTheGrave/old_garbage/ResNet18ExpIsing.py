...
from old_garbage.Models import BaseModel, MidModel, NeuralIsingTournament, copy_matching_parameters
from old_garbage.Training_testing_old import joint_train_all_mixup, joint_eval_all, joint_train_all_ising, ConvergenceMonitor, joint_eval_all_var
...
def main(num_epochs, path_mod):
    ...
    # optimizer_base = torch.optim.AdamW(base_model.parameters(), lr=0.1)
    # optimizer_mid = torch.optim.AdamW(mid_model.parameters(), lr=0.1)
    optimizer_tournament = torch.optim.Adam(tournament_model.parameters(), lr=0.1)
