...
from old_garbage.Models import BaseModel, MidModel, copy_matching_parameters, TournamentModel #Dropout
from old_garbage.Training_testing_old import joint_train_all_separate, joint_eval_all, ConvergenceMonitor
...
def main(num_epochs, path_mod):
    ...
    base_model = BaseModel(class_count, device = device, backbone='resnet18ext').to(device)
    mid_model = MidModel(class_count, device = device, backbone='resnet18ext').to(device)
    copy_matching_parameters(base_model, mid_model)
    tournament_model = TournamentModel(class_count, device = device, backbone='resnet18ext').to(device)
    ...
    ckpt_base = f'ckpts/cifar100/resnet18ext{_path_mod}'
    monitor = ConvergenceMonitor(patience=3, mode='max', save_dir=ckpt_base)

    print("Starting joint training...")
    for epoch in range(num_epochs):
        print(f"Epoch {epoch}")
        joint_train_all_separate(device, train_loader, models, class_count, temps = [1,1, 10], lbda = 1.0)
